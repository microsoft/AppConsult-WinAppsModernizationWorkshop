### Exercise 6 - Enable a CI/CD pipeline for your Windows application with Azure DevOps
Adopting a DevOps mindset for your software project is becoming more and more important nowadays. Thanks to a DevOps process, you can deliver faster and more often; you can resolve problems and address feedbacks faster; you can focus on the code rather than on deployment. However, when it comes to adopt a DevOps process for a Windows desktop application, there might be some challenges compared to a web application. Windows applications, in fact, run locally on the customer's machine and, as such, we might have 1.000 or 1.000.000 copies of our application running all over the world. How can we quickly reach all our customers with the most recent version of our application?

In this exercise you will learn how to start with Azure DevOps for free, how to define builds to automatically run whenever a team member checks in code changes and to build pipelines to package the application to MSIX after the build runs. Then, using MSIX and a Windows 10 feature called AppInstaller, you will learn how you can deliver automatic updates to all your customers.
...
### Exercise 6 Task 1 - Integrate Contoso Expenses with Azure Repos
...
### Exercise 6 Task 2 - Create your first pipeline
...
### Exercise 6 Task 3 - Set the right image for the hosted agent
As the **Contoso Expenses** project is built on top of .NET Core 3.0, which is supported starting from Visual Studio 2019, we need to use the most recent Windows image, otherwise the compilation will fail.

1. Identify the section titled `pool`.
2. Make sure that the `vmImage` value is set to `windows-latest`.

![](../Manual/Images/AzureDevOpsPipelineChangeVmImage.png)


### Exercise 6 Task 4 - Build for the right CPU architectures
...
### Exercise 6 Task 5 - Install the .NET Core 3.0 SDK on the hosted agent
At the time of writing, the .NET Core 3.0 SDK isn't installed by default on the Azure Pipeline hosted agent yet. We need to manually install it before triggering the build process. We can easily do it thanks to one of the many available task included in Azure DevOps called **UseDotNet**.
...
### Exercise 6 Task 6 - Remove the NuGet tasks
...
### Exercise 6 Task 7 - Configure the build
...
2. We need to disable the package signing. By default, in fact, MSIX packages are signed with a self-signing certificate generated by Visual Studio during the build process. However, signing the package during the build process isn't a good practice because we would need to upload the certificate to the repository. This means that every developer who is working on the project will have access to the  certificate, increasing the risk of identity theft. As such, the recommended approach is to sign the package during the release process and store it in a safe way. We're going to see how to do it in one of the next tasks. For the moment, let's just disable the signing during the compilation, by setting the `AppxPackageSigningEnabled` property to `false` in the build process. 
...
### Exercise 6 Task 9 - Testing the build
...
### Exercise 6 Task 10 - Change the build version number
...
13. Change the default settings as below:
...
### Exercise 6 Task 11 - Create a signing certificate
...
5. Run the following command in the PowerShell prompt to export the certificate:
    ...
    - The third line defines the path where to export the PFX file. This scripts exports the file on the desktop with the name **AppConsult.pfx**. Feel free to change the path and the file name, just take note of where you have saved it because you will need it later.
...
### Exercise 6 Task 12 - Create the release pipeline - Signing the package
...
### Exercise 6 Task 13 - Create the release pipeline - Deploy to a website with AppInstaller
...
### Exercise 6 Task 14 - Test automatic updates
...
### Exercise 6 Task 15 - Create a Visual Studio App Center account
...
### Exercise 6 Task 16 - Create the release pipeline - Deploy to Visual Studio App Center
